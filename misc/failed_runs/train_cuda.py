...
env_fns = [lambda: FrameStackObservation(
    ...
    AtariPreprocessing(gym.make("BreakoutDeterministic-v4", frameskip=1), frame_skip=4, screen_size=84, grayscale_obs=True),
    stack_size=4
...
def choose_action(state, epsilon):
    if random.random() < epsilon:
        ...
        state_tensor = state_tensor.float().unsqueeze(0)
...
for episode in range(num_episodes):
    ...
    states = np.array(states) # Output shape: envs * 4 * 84 * 84
    episode_reward = np.zeros(num_envs) # Output shape: envs * 1
    steps = np.zeros(num_envs) # Output shape: envs * 1
    is_done = np.zeros(num_envs, dtype=bool) # Output shape: envs * 1
    ...
    while not all(is_done):
        ...
        # Actions shape: envs * 1
        actions = [choose_action(states[i], epsilon) for i in range(num_envs)]
        next_states, rewards, is_dones, _, infos = env.step(actions)
        next_states = np.array(next_states) # Output shape: envs * 4 * 84 * 84
        # Output shape: envs * 1
        rewards = np.clip(np.array(rewards), -1, 1)
        
        for i in range(num_envs):
            # Add a tuple of the current step's state
            replay_buffer.append((states[i], actions[i], rewards[i], next_states[i], is_dones[i]))
            ...
            if len(replay_buffer) > batch_size:
                ...
                if steps[i] % 20 == 0:
                    ...
                    with open("loss.txt", "a") as f:
                        f.write(f"episode: {episode}, step: {steps[i]}, loss: {loss.item()}\n")
                scaler.step(optimiser)
                scaler.update()
            
            if steps[i] % target_net_update_freq == 0:
                target_net.load_state_dict(dqn.state_dict())
    ...
    with open("loss.txt", "a") as f:
        f.write(f"Episode {episode} finished with reward {episode_reward}\n")
