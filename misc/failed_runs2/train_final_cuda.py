...
hyperparams = {
    ...
    "buffer_min_size": 75000,  # Minimum buffer size before training starts
...
optimiser = optim.AdamW(dqn.parameters(), lr=hyperparams["learning_rate"], weight_decay=0.0)
# loss_fn = nn.MSELoss()
    ...
    while not is_done:
        ...
        if len(replay_buffer) > batch_size and not buffer_filling:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"Warning: Loss is NaN or Inf at step {total_steps}")
                wandb.log({"warning": "Loss is NaN or Inf"}, step=total_steps)
                continue
